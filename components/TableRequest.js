...
const TableRequest = ({ data, pagination }) => {
      ...
      <DataTable.Header style={{ paddingHorizontal: 0 }}>
        {Object.keys(data[0]).map((key, index) => (
          <DataTable.Title
            ...
            style={{
              flex: index == 0 ? 0.4 : index == 3 || index == 4 ? 0.7 : 1,
              marginHorizontal: 0,
      ...
      <DataTable.Row key={"Total"} style={{ paddingHorizontal: 0 }}>
        {Object.keys(data[0]).map((key, index) => (
          <DataTable.Cell
            ...
            style={{
              flex: index === 0 ? 1 : index === 5 ? 1 : 0.6, // Ajuste de flex para la última celda
              marginHorizontal: 0,
              justifyContent: "center",
              borderColor: "white",
              borderTopWidth: 2,
          ...
          >
            {index === 0 ? "Total" : index === 5 ? `${sumColumn("Total")}` : ""}
          </DataTable.Cell>
        ))}
      </DataTable.Row>
