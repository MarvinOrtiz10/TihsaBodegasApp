...
import { AcuerdoVenta, Pedidos } from "../../settings/EndPoints.js";
...
const PickingOrders = () => {
  ...
  const baseUrl = Pedidos.EndPoint;
  ...
  const cargarInformacion = () => {
    setIsLoading(true);
    axios.get(`${baseUrl}/${codEmp}/${usuario}`).then((response) => {
  ...
  const renderItemOrder = ({ item }) => {
    return (
      <TouchableHighlight
        ...
        onPress={() => handlePressCheckin(item)}
        ...
        >
...

          {
            ...
            loadingOrders ? (
              ...
              <View style={{ flex: 1, paddingTop: 16, alignItems: 'center', justifyContent: 'center', }}>
                <ActivityIndicator size="large" color="#FED30B" />
                <Text style={{ color: "gray", fontSize: 18 }}>Cargando pedidos</Text>
              </View>
            ) : (
              <Block style={{ padding: 24 }}>
                <Text
                ...
                >
                  No tiene pedidos pendientes de facturar.
                </Text>
              </Block>
            )
          }
